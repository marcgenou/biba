...
echo "phpmyadmin phpmyadmin/app-password-confirm password biba" | debconf-set-selections
echo "phpmyadmin phpmyadmin/mysql/admin-pass password biba" | debconf-set-selections
echo "phpmyadmin phpmyadmin/mysql/app-pass password biba" | debconf-set-selections
echo "phpmyadmin phpmyadmin/reconfigure-webserver multiselect apache2" | debconf-set-selections
apt-get install -y build-essential libssl-dev libyaml-dev libreadline-dev openssl curl git-core zlib1g-dev bison libxml2-dev libxslt1-dev libcurl4-openssl-dev libsqlite3-dev ruby2.2 ruby2.2-dev curl git libmysqlclient-dev mysql-server nodejs apache2 libapache2-mod-passenger phpmyadmin
...
echo "    <Directory /usr/share/phpmyadmin>" >> /etc/apache2/sites-available/testapp.conf
echo "        PassengerEnabled off" >> /etc/apache2/sites-available/testapp.conf
echo "        AllowOverride all" >> /etc/apache2/sites-available/testapp.conf
...
echo "        Allow from all" >> /etc/apache2/sites-available/testapp.conf
echo "        Options -MultiViews" >> /etc/apache2/sites-available/testapp.conf
